...
// +--------------+--------------------+--------------------------------------+
// |  2023.06.26  | Takahiro Miyashiro | Loop striping (3 expansions) applied |
// |              |                    | to eval_interacting_pairs_deriv      |
...
fl eval_interacting_pairs_deriv(const precalculate& p, fl v, const interacting_pairs& pairs, const vecv& coords, vecv& forces) { // adds to forces  // clean up
	const fl cutoff_sqr = p.cutoff_sqr();
	fl e = 0;
//        for(std::size_t VINA_MACROS_TMP = (pairs).size(), (i) = 0; (i) < VINA_MACROS_TMP; ++(i)){
        std::size_t VINA_MACROS_TMP = (pairs).size();
        std::size_t i, j, end_kernel, unroll_num;
        unroll_num = 3;
        end_kernel = ( (VINA_MACROS_TMP - 0) % unroll_num != 0 ) ? (VINA_MACROS_TMP - unroll_num) : VINA_MACROS_TMP;
        //unroll loop
        for(i=0; i<end_kernel; i+=unroll_num){
          const interacting_pair& ip = pairs[i];
          const interacting_pair& ip_2 = pairs[i+1];
          const interacting_pair& ip_3 = pairs[i+2];
          vec r = coords[ip.b] - coords[ip.a]; // a -> b
          vec r_2 = coords[ip_2.b] - coords[ip_2.a]; // a -> b
          vec r_3 = coords[ip_3.b] - coords[ip_3.a]; // a -> b
          fl r2 = sqr(r);
          fl r2_2 = sqr(r_2);
          fl r2_3 = sqr(r_3);
          if(r2 < cutoff_sqr && r2_2 < cutoff_sqr && r2_3 < cutoff_sqr) {
            pr tmp = p.eval_deriv(ip.type_pair_index, r2);
            pr tmp2 = p.eval_deriv(ip_2.type_pair_index, r2_2);
            pr tmp3 = p.eval_deriv(ip_3.type_pair_index, r2_3);
            vec force; force = tmp.second * r;
            vec force2; force2 = tmp2.second * r_2;
            vec force3; force3 = tmp3.second * r_3;
            curl(tmp.first, force, v);
            curl(tmp2.first, force2, v);
            curl(tmp3.first, force3, v);
            e += tmp.first;
            e += tmp2.first;
            e += tmp3.first;
            forces[ip.a] -= force;
            forces[ip_2.a] -= force2;
            forces[ip_3.a] -= force3;
            forces[ip.b] += force;
            forces[ip_2.b] += force2;
            forces[ip_3.b] += force3;
          } else if(r2 < cutoff_sqr && r2_2 < cutoff_sqr) {
            pr tmp = p.eval_deriv(ip.type_pair_index, r2);
            pr tmp2 = p.eval_deriv(ip_2.type_pair_index, r2_2);
            vec force; force = tmp.second * r;
            vec force2; force2 = tmp2.second * r_2;
            ...
            forces[ip.b] += force;
            forces[ip_2.b] += force2;
          } else if(r2 < cutoff_sqr && r2_3 < cutoff_sqr) {
            pr tmp = p.eval_deriv(ip.type_pair_index, r2);
            pr tmp3 = p.eval_deriv(ip_3.type_pair_index, r2_3);
            vec force; force = tmp.second * r;
            vec force3; force3 = tmp3.second * r_3;
            ...
            forces[ip.b] += force;
            forces[ip_3.b] += force3;
          } else if(r2_2 < cutoff_sqr && r2_3 < cutoff_sqr) {
            pr tmp2 = p.eval_deriv(ip_2.type_pair_index, r2_2);
            pr tmp3 = p.eval_deriv(ip_3.type_pair_index, r2_3);
            vec force2; force2 = tmp2.second * r_2;
            vec force3; force3 = tmp3.second * r_3;
